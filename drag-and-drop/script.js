...
function createItemEl(columnEl, column, item, index) {
  // List Item
  const listEl = document.createElement("li"); //creating list element
  listEl.classList.add("drag-item");
  listEl.textContent = item;
  listEl.draggable = true; // making the item is draggable
...
function updateDOM() {
  ...
  completeListArray.forEach((completItem, idx) => {
    createItemEl(completeList, 2, completItem, idx);
  });
...
function addToColumn(column) {
  ...
  const selectedColumnArray = listArrays[column];
  selectedColumnArray.push(itemText);
...
function dragEnter(column) {
  listColumns[column].classList.add("over"); // add visual effects when item is being dragged
  currentColumn = column;
}

// Column Allows for Item to Drop
function allowDrop(e) {
  e.preventDefault(); // By default, data/elements cannot be dropped in other elements. To allow a drop, we must prevent the default handling of the element.
}
